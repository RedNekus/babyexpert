$(document).ready(function() {
	...
	var b_gridUrl	 	= baseUrl+"load_rezerv";
	...
	table.jqGrid({
		...
		autowidth: true,
		height: height,		
        colNames:[' ','№з','Менеджер','Наименование товара','Телефон','Кол-во','Дата заказа','Ожидают до', 'Примечание менеджера'],
        colModel :[	
			...
			{name:'date_rezerv', 	index:'date_rezerv', 	width:45,  align:'center', search:false, formatter: getDayWeek},	
			{name:'comment_m', 		index:'comment_m', 		width:150, align:'left',   search:false, },	
			],
        pager: pager,
        sortname: 'date_rezerv',
		toolbar: [true,"top"],
        sortorder: 'desc',
		rowNum:100,	
		viewrecords: true,	
			...
			if(id){
				$.ajax({
					...
					success:function (res) {//возвращаемый результат от сервера
						...
						tableTovar.jqGrid('setGridParam',{url: urlTovar+"?id_client="+id+"&rezerv=1",page:1,datatype: 'json',mtype: 'GET'}).trigger("reloadGrid");
					},
