$(document).ready(function() {
	var baseUrl			= "/adminpanel/banners_small/";
	...
	$.ajax({type: "POST",url: b_dataAccess,dataType: "json",data: "",
		success:function (res) {//возвращаемый результат от сервера
			...
			if (access['banners_add']==1) add = '<div id="adddata" class="button add"></div>';
			else add = '<div class="button add noactive"></div>';
			
			if (access['banners_edit']==1) edit = '<div id="editdata" class="button edit"></div>';
			else edit = '<div class="button edit noactive"></div>';
			
			if (access['banners_del']==1) del = '<div id="deldata" class="button del"></div>';
	...
	table.jqGrid({
		...
		autowidth: true,
		height: height,		
        colNames:['Код', 'Название', 'Ссылка', 'Активен','Приоритет'],
        colModel :[
			{name:'id', 		index:'id', 			width:15, 	align:'center', 	search:false, 	editable:true, 	edittype:"text" },
            {name:'name', 		index:'name', 			width:250, 	align:'left', 		search:true, 	editable:true, 	edittype:"text" },
			{name:'path', 		index:'path', 			width:250, 	align:'left', 		search:false, 	editable:true, 	edittype:"text" },
		...
		ondblClickRow: function(id) {
			if (access['banners_edit']==1) gridEdit();
		},
					...
					success:function (res) {//возвращаемый результат от сервера
						for(var key in res){ 
							if (key == "active") {
								...
								if (key == "url") $('#banners_insert').html("<img src='"+res['url']+"' />");							
								$('#val_'+key).val(res[key]);
							}
						}
					$("#valueBanners_file").val("");	
					},
