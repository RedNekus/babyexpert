$(document).ready(function() {
	...
	$.ajax({type: "POST",url: b_dataAccess,dataType: "json",data: "",
		success:function (res) {//возвращаемый результат от сервера
			...
			if (access['sklad_add']==1) add = '<div id="adddata" class="button add"></div>';
			else add = '<div class="button add noactive"></div>';
			
			if (access['sklad_edit']==1) edit = '<div id="editdata" class="button edit"></div>';
			else edit = '<div class="button edit noactive"></div>';
			
			if (access['sklad_del']==1) del = '<div id="deldata" class="button del"></div>';
	...
	table.jqGrid({
		...
		ondblClickRow: function(id) {
			if (access['sklad_edit']==1) gridEdit();
		},
