$(document).ready(function() {
	var baseUrl			= "/adminpanel/competitors/";
	...
	$.ajax({type: "POST",url: b_dataAccess,dataType: "json",data: "",
		success:function (res) {//возвращаемый результат от сервера
			...
			if (access['competitors_add']==1) add = '<div id="adddata" class="button add"></div>';
			else add = '<div class="button add noactive"></div>';
			
			if (access['competitors_edit']==1) edit = '<div id="editdata" class="button edit"></div>';
			else edit = '<div class="button edit noactive"></div>';
			
			if (access['competitors_del']==1) del = '<div id="deldata" class="button del"></div>';
	...
	table.jqGrid({
		...
		autowidth: true,
		height: height,		
        colNames:['Код','Название','Телефон','Сайт','Активен'],
        colModel :[
			{name:'id', 		index:'id', 		width:15, 	align:'center', 	search:false, 	},
            {name:'name', 		index:'name', 		width:300, 	align:'left', 		search:true, 	},
			{name:'phone', 		index:'phone', 		width:150, 	align:'left', 		search:true, 	},
			{name:'site', 		index:'site', 		width:150, 	align:'left', 		search:true, 	},
			{name:'active', 	index:'active', 	width:450, 	align:'left', 		search:false, 	},
			],
        pager: pager,
        sortname: 'id',
		toolbar: [true,"top"],
		...
		ondblClickRow: function(id) {
			if (access['competitors_edit']==1) gridEdit();
		},
