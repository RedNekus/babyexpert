$(document).ready(function() {
	...
	$.ajax({type: "POST",url: b_dataAccess,dataType: "json",data: "",
		success:function (res) {//возвращаемый результат от сервера
			...
			if (access['valute_add']==1) add = '<div id="adddata" class="button add"></div>';
			else add = '<div class="button add noactive"></div>';
			
			if (access['valute_edit']==1) edit = '<div id="editdata" class="button edit"></div>';
			else edit = '<div class="button edit noactive"></div>';
			
			if (access['valute_del']==1) del = '<div id="deldata" class="button del"></div>';
	...
	table.jqGrid({
		...
		ondblClickRow: function(id) {
			if (access['valute_edit']==1) gridEdit();
		},
