$(document).ready(function() {
	var baseUrl			= "/adminpanel/kontragenty/";
	...
	$.ajax({type: "POST",url: b_dataAccess,dataType: "json",data: "",
		success:function (res) {//возвращаемый результат от сервера
			...
			if (access['kontragenty_add']==1) add = '<div id="adddata" class="button add"></div>';
			else add = '<div class="button add noactive"></div>';
			
			if (access['kontragenty_edit']==1) edit = '<div id="editdata" class="button edit"></div>';
			else edit = '<div class="button edit noactive"></div>';
			
			if (access['kontragenty_del']==1) del = '<div id="deldata" class="button del"></div>';
	...
	table.jqGrid({
		...
		autowidth: true,
		height: height,		
        colNames:['Код','Тип','Название','Активен','Дата создания'],
        colModel :[
			{name:'id', 			index:'id', 			width:15, 	align:'center', 	search:false, 	editable:false, 	edittype:"text" },
            {name:'id_tip',			index:'id_tip', 		width:55, 	align:'left', 		search:false, 	editable:false, 	edittype:"text" },
		...
		ondblClickRow: function(id) {
			if (access['kontragenty_edit']==1) gridEdit();
		},
