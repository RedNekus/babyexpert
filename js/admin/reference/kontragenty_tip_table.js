$(document).ready(function() {
	var baseUrl			= "/adminpanel/kontragenty_tip/";
	...
	$.ajax({type: "POST",url: b_dataAccess,dataType: "json",data: "",
		success:function (res) {//возвращаемый результат от сервера
			...
			if (access['kontragenty_tip_add']==1) add = '<div id="adddata" class="button add"></div>';
			else add = '<div class="button add noactive"></div>';
			
			if (access['kontragenty_tip_edit']==1) edit = '<div id="editdata" class="button edit"></div>';
			else edit = '<div class="button edit noactive"></div>';
			
			if (access['kontragenty_tip_del']==1) del = '<div id="deldata" class="button del"></div>';
	...
	table.jqGrid({
		...
		ondblClickRow: function(id) {
			if (access['kontragenty_tip_edit']==1) gridEdit();
		},
		...
		gsr = table.jqGrid('getGridParam','selrow');
			if(gsr){
				$.ajax({
					...
					success:function (res) {//возвращаемый результат от сервера
						for(var key in res){ 
							if ($.inArray(key, Array('active', 'couriers_show')) != -1) {	
								if (res[key] != 0) 	 {	$('#val_'+key).attr('checked', true);} else { $('#val_'+key).attr('checked', false); }						
							} else {
								if (key == "id_tip_operation") {
									var arr = res[key].split(",");
									$(".ch-clear").attr({'checked': false});
									for(var ley in arr) {										
										$('#val_ch_'+arr[ley]).attr('checked', true);	
									}
								}						
