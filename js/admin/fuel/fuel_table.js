$(document).ready(function() {
	var baseUrl			= "/adminpanel/fuel/";
	var accessURL		= "/adminpanel/adminaccess/";

	var pager 			= '#le_tablePager';
	var table 			= $('#le_table');
	var dialogEdit 		= $("#dialog-edit");
	var dialogDel 		= $("#dialog-del");
	var b_gridUrl		= baseUrl+"load";
	var b_editUrl 		= baseUrl+"edit";
	var b_openUrl 		= baseUrl+"open";
	var b_openLastUrl 	= baseUrl+"open_last";
	var b_dataAccess	= accessURL+"access";
	var tree			= $("#tree_connection");
	var id_tree;

	var height 			= $(window).height()-135;
	window.onresize = function () { height = $(".ui-jqgrid-bdiv").height($(window).height()-135);} 

	$.ajax({type: "POST",url: b_dataAccess,dataType: "json",data: "",
		success:function (res) {//возвращаемый результат от сервера
	
			access = res;
			$('#t_le_table').height(28);

			if (access['fuel_add']==1) add = '<div id="adddata" class="button add"></div>';
			else add = '<div class="button add noactive"></div>';
			
			if (access['fuel_edit']==1) edit = '<div id="editdata" class="button edit"></div>';
			else edit = '<div class="button edit noactive"></div>';
			
			if (access['fuel_del']==1) del = '<div id="deldata" class="button del"></div>';
			else del = '<div class="button del noactive"></div>';
					
			date_ot =  '<label>ДАТА ОТ </label><input type="text" id="date_ot">';
			date_do =  '<label>ДО </label><input type="text" id="date_do">';
			date_btn =  '<a href="" class="date_btn btn-cur">Найти</a>';
						
			input_html = '<div class="block-input"><input type="text" class="km"/><input type="text" class="total_cena"/></div>';
				
			$('#t_le_table').append('<div class="toolbar_top" id="select_form"><div class="toolbar-block">' + add + edit + del + date_ot + date_do + date_btn + input_html + '</div></div>');		

			// Обработка события кнопки Добавить	
			$("#adddata").click(function(){
				gridAdd();
			});
			
			// Обработка события кнопки Редактировать	
			$("#editdata").click(function(){
				gridEdit();
			});

			// Обработка события кнопки Удалить	
			$("#deldata").click(function(){
				gridDelete();
			});

		}						
	});

	// при нажатии на ссылку на дереве
	tree.delegate(".linkrel", "click", function(){
		var id_tree = $(this).attr('id');
		$("#val_id_tree").val(id_tree);
		tree.find(".active").removeClass("active");
		$(this).addClass('active');	
		
		if ($("#valueTree_id_courier").val() != 0) str = "&id_courier="+$("#valueTree_id_courier").val();
		else str = '';
		
		
		table.jqGrid('setGridParam',{url: b_gridUrl+"?id_tree="+id_tree+str,page:1}).trigger("reloadGrid");
	});
			
	table.jqGrid({
		url:b_gridUrl,
		datatype: 'json',
        mtype: 'GET',
		autowidth: true,
		height: height,		
        colNames:['Курьер','Вид топлива','км','Цена за л.','Примечение','Дата'],
        colModel :[
			{name:'id_courier',	index:'id_courier', width:75, 	align:'center', search:false, },
			{name:'id_tree', 	index:'id_tree', 	width:50, 	align:'center', search:false, },			
            {name:'km', 		index:'km', 		width:100, 	align:'center', search:false, },			
            {name:'cena', 		index:'cena', 		width:100, 	align:'center', search:false, },
			{name:'comment', 	index:'comment', 	width:350, 	align:'left', 	search:false, },
			{name:'date_create',index:'date_create',width:50, 	align:'center', search:false, },
			],
        pager: pager,
        sortname: 'date_create',
		toolbar: [true,"top"],
        sortorder: 'desc',
		rowNum:50,
		viewrecords: true,		
		ondblClickRow: function(id) {
			if (access['fuel_edit']==1) gridEdit();
		},
		gridComplete: function() {
		
			$( "#date_ot" ).datepicker({
				changeMonth: true,
				dateFormat: 'yy-mm-dd',
				changeYear: true
			});		
			
			$( "#date_do" ).datepicker({
				changeMonth: true,
				dateFormat: 'yy-mm-dd',
				changeYear: true
			});	
		
			get_total_sum();			

		},
		rowList:[50,100,150,200],
		rownumbers: true,
        rownumWidth: 30,
		footerrow: true,
		userDataOnFooter: true,		
		editurl:b_editUrl
        }).jqGrid('navGrid', pager,{refresh: true, add: false, del: false, edit: false, search: true, view: false},
					{}, // параметры редактирования
					{}, // параметры добавления
					{}, // параметры удаления
					{closeOnEscape:true, multipleSearch:false, closeAfterSearch:true}, // параметры поиска
					{} /* параметры просмотра */
				);
	
	$("#t_le_table").delegate(".date_btn", "click", function(){
		var id_tree = tree.find(".active").attr('id'),
			date_ot = $("#date_ot").val(),
			date_do = $("#date_do").val();
			if (id_tree)
			table.jqGrid('setGridParam',{url: b_gridUrl+'?id_tree='+id_tree+'&date_ot='+date_ot+'&date_do='+date_do,page:1,datatype: 'json',mtype: 'GET'}).trigger("reloadGrid");
			else 
			alert('Необходимо выбрать топливо!');
		return false;
	});	
		
	function get_total_sum() {
		var myUserData = table.getGridParam('userData');
		$(".km").val(myUserData.km);		
		$(".total_cena").val(myUserData.total_cena);		
	}
	
	// Функция добавление итема в таблицу		
	function gridAdd(){
		$("#form").trigger("reset");	
		$("#action_pole").attr({value: "add"});
		$.ajax({
			type: "POST",
			dataType: "json",
			url: b_openLastUrl,
			success:function (res) {//возвращаемый результат от сервер							
				$('#val_cena').val(res);
			},
			error: function(jqXHR, textStatus, errorThrown) {alert(textStatus);}						
		});		
		
		dialogEdit.dialog( "open" );
		}

	// Функция редактирование итема в тиблице	
	function gridEdit(){
		gsr = table.jqGrid('getGridParam','selrow');
			if(gsr){
				$.ajax({
					type: "POST",
					dataType: "json",
					url: b_openUrl,
					data: "id="+gsr,
					success:function (res) {//возвращаемый результат от сервера
						for(var key in res){ 
							if (key == "active") {
								if (res[key] != 0) 	 {	$('#val_'+key).attr('checked', true);} else { $('#val_'+key).attr('checked', false); }						
							} else {							
								$('#val_'+key).val(res[key]);
							}
						}
					},
					error: function(jqXHR, textStatus, errorThrown) {alert(textStatus);}						
					});
				$("#action_pole").attr({value: "edit"});
				dialogEdit.dialog( "open" );
			} else {
				alert("Пожалуйста выберите запись!")
			}
	}	

	// Функция удаления итема из таблицы
	function gridDelete(){
		var gr = table.jqGrid('getGridParam','selrow');
			if( gr != null ) {
				dialogDel.dialog( "open" );
					$("#del_id").val(gr);
			} else alert("Пожалуйста выберите запись!");
	}	

	$("#valueTree_id_courier").change(function(){
		var id_courier = $(this).val(),
			id_tree = tree.find(".active").attr('id');
			if (id_tree) str = "&id_tree="+id_tree;
			else str = "";
		table.jqGrid('setGridParam',{url: b_gridUrl+"?id_courier="+id_courier+str,page:1}).trigger("reloadGrid");
	});
	
	// Инициализация формы добавления и редактирования данных					
	dialogEdit.dialog({
		autoOpen: false,
		width: 570,
		minHeight: 'auto',
		title: "Свойства",
		buttons: [
				{
				text: "Сохранить",
				click: function() {
					var str = $("#form").serialize();
						$.ajax({
							type: "POST",
							url: b_editUrl,
							data: str,
							});
						table.trigger("reloadGrid");
						$( this ).dialog( "close" );						
					}
				},
				{
				text: "Отмена",
				click: function() {
					$( this ).dialog( "close" );
					}
				}
			]
	});

	function DelItemCatalog() {
		var id = $("#del_id").val();
			$.ajax({
				type: "POST",
				url: b_editUrl,
				data: "del_id="+id,
			});
			table.trigger("reloadGrid");
			$( this ).dialog( "close" );						
	}
		
	dialogDel.dialog({
		autoOpen: false,
		title: "Подтверждение",
		width: 400,
		buttons: [
			{
				text: "Да",
				click: DelItemCatalog
			},
			{
				text: "Нет",
				click: function() {
					$( this ).dialog( "close" );
				}
			}
		]
	});	
	
});