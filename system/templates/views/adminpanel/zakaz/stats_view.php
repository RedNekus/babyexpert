...
<script type="text/javascript" src="/js/admin/zakaz/grid_stats.js"></script>
			...
			<div id="tabs-content">
				...
				<table class="table-tabs-content char">
					<tr class="v-top">
						<td>
							<fieldset>
								<div><b class="cur-pointer">Операция:</b></div>
								<input type="checkbox" name="our_sell" id="our_sell" /> <label class="norm-label" for="our_sell">Наши продажи</label> <br/>
								<input type="checkbox" name="diler_urozhay" id="diler_urozhay" /> <label class="norm-label" for="diler_urozhay">Дилеры урожай</label> <br/>
								<input type="checkbox" name="diler_teplich" id="diler_teplich" /> <label class="norm-label" for="diler_teplich">Дилеры тепличное</label> <br/>					
							</fieldset>						
						</td>
						<td>					
							<fieldset>
								<div><b class="cur-pointer">Дата:</b></div>
					...
					<tr class="v-top">	
						<td>	
							<fieldset>
								<div><b class="cur-pointer">Дилеры:</b></div>
								<?php $items = Database::getRows(get_table('registration'),'id','asc',false,'diler > 0');
								foreach($items as $item): ?>
								<input type="checkbox" name="diler[]" class="tmp-t<?php echo $item['id']; ?>" value="<?php echo $item['id']; ?>" /><label class="norm-label" for=""><?php echo $item['name']; ?></label> <br/>
								<?php endforeach; ?>
							</fieldset>	
						</td>						
						<!--<td>
							<fieldset>
								<div><b class="cur-pointer">Поставщики:</b></div>
								<?php //$items = Database::getRows(get_table('kontragenty'),'id','asc',false,'id_tip = 2');
								//foreach($items as $item): ?>
								<input type="checkbox" name="suppliers[]" class="tmp-t<?php //echo $item['id']; ?>" value="<?php //echo $item['id']; ?>" /><label class="norm-label" for=""><?php //echo $item['name']; ?></label> <br/>
								<?php //endforeach; ?>
							</fieldset>						
						</td>-->				
						<td>	
							<fieldset>
								<div><b class="cur-pointer">Менеджеры:</b></div>
								<?php $items = Database::getRows(get_table('kontragenty'),'id','asc',false,'id_tip = 4');
								foreach($items as $item): ?>
								<input type="checkbox" name="manager[]" class="tmp-t<?php echo $item['id']; ?>" value="<?php echo $item['id']; ?>" /><label class="norm-label" for=""><?php echo $item['name']; ?></label> <br/>
								<?php endforeach; ?>
							</fieldset>	
						</td>
						<td>
