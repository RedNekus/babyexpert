...
class Database {
	...
	public static function getRows($table, $order_name='id', $order='asc', $limit=false, $where = '1', $group='') {
        
		if (!empty($group)) { $group = " GROUP BY {$group}"; }	
	
		$sql = "SELECT * FROM $table WHERE $where $group ORDER BY $order_name $order";
