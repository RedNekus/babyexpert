...
class Akcii_Controller {
	private $_config, $_content, $_table, $_table_lng;

	public function __construct() {
		$this->_config = Config::getParam('modules->akcii');
		$this->_table = get_table('akcii');
		$this->_table_lng = get_table_lng('akcii');
		$this->_content['title'] = 'Акции сайта';
	}

	public function defaultAction() {
	
		$this->_content['content'] = Render::view('adminpanel/akcii/list');
	...
	public function editAction() {
		...
		if (isset($_POST['action']) and $_POST['action']=="add") {
			...
			if (!$error) {	
				...
				$id_akcii_lng = Database::getLastId($this->_table);	
			
				$data_lng = array(
					'id_akcii_lng' => $id_akcii_lng,
					'id_language' => 2,
		...
		if (isset($_POST['del_id']))  {
			...
			Database::delete($this->_table_lng,$id,'id_akcii_lng');
	...
	public function openAction() {
		...
		$data['language'] = Database::getRow($this->_table_lng, $_POST['id'], 'id_akcii_lng', $id_language);
		echo json_encode($data);
	}
