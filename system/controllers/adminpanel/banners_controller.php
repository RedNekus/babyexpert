...
class Banners_Controller {
	...
	public function __construct() {
		Load::model('Tree');
		$this->_config 		= Config::getParam('modules->banners');
		$this->_table     	= get_table('banners');
		$this->_table_lng 	= get_table_lng('banners');		
		$this->_content['title'] = 'Баннеры';
	}

	public function defaultAction() {
	
		$this->_content['content'] = Render::view(
				'adminpanel/banners/list',array(
						'img_size' => $this->_config['image']['small'],
						'trees' => Tree::getTreesForSite("pid=1")
				)
			);
	...
	public function editAction() {
		...
		if (isset($_POST['action']) and $_POST['action']=="add") {
			...
			if (!$error) {	
				...
				$id_banners_lng = Database::getLastId();	
			
				$data_lng = array(
					'id_banners_lng' => $id_banners_lng,
					'id_language' => 2,
		...
		if (isset($_POST['del_id']))  {
			...
			Database::delete($this->_table_lng,$id,'id_banners_lng');
	...
	public function openAction() {
		...
		$data['url'] = $this->_config['image']['big']['path'].$data['image'];
		$data['language'] = Database::getRow($this->_table_lng, $_POST['id'], 'id_banners_lng', $id_language);
		echo json_encode($data);
	}		
