...
class Banners_left_Controller {
	private $_config, $_content, $_table, $_table_lng;

	public function __construct() {
		$this->_config = Config::getParam('modules->banners_left');
		$this->_table  = get_table('banners_left');			
	...
	public function defaultAction() {
		...
		$this->_content['content'] = Render::view(
				'adminpanel/banners/list_left',array(
					'img_size' => $this->_config['image']
				)
			);
	...
	public function openAction() {
		...
		$data['url'] = $this->_config['image']['path'].$data['image'];
		echo json_encode($data);
	}
