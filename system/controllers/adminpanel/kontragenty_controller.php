...
class Kontragenty_Controller {
	private $_content, $_table;
	
	public function __construct() {
		$this->_table = get_table('kontragenty');
		$this->_table_tip = get_table('kontragenty_tip');
		$this->_content['title'] = 'Контрагенты';
	}

	public function defaultAction() {
	
		$id_tips = Database::getRows($this->_table_tip);
	
		$this->_content['content'] = Render::view('adminpanel/reference/kontragenty',
			array(
				'id_tips' => $id_tips
			));
    ...
    public function loadAction() {
		...
		foreach($items as $item) {
			...
			$data['rows'][$i]['cell'][] = Database::getField($this->_table_tip,$item['id_tip']);
	...
	public function openAction() {
		$carriers = array();
		$carriers = Database::getRow($this->_table,$_POST['id']);
